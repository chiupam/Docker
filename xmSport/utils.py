...
def get_data(_step):
    ...
    data_json = re.sub(find_date.findall(data_json)[0], time.strftime("%F"), str(data_json))
    data_json = re.sub(find_step.findall(data_json)[0], _step, str(data_json))
    return data_json


async def login(_user, _password):
    app_token, userid = None, None
    try:
        async with aiohttp.ClientSession() as client:
            url = f"https://api-user.huami.com/registrations/+86{_user}/tokens"
            headers = {
            ...
            response = await client.post(url, data=data, headers=headers, allow_redirects=False)
            ...
            userid = response["token_info"]["user_id"]
...
async def change(_user, _app_token, _userid, _step):
    if _step == '':
        print("已设置为随机步数（18000-25000）")
        _step = str(random.randint(18000, 25000))
    _step = _step.split('-')
    if len(_step) == 2:
        _step = str(random.randint(int(_step[0]), int(_step[1])))
    url = f'https://api-mifit-cn.huami.com/v1/data/band_data.json?&t={round(time.time() * 1000)}'
    head = {
        "apptoken": _app_token,
        "Content-Type": "application/x-www-form-urlencoded"
    }
    data = f'userid={_userid}&' \
           f'last_sync_data_time=1597306380&' \
           f'device_type=0&' \
           f'last_deviceid=DA932FFFFE8816E7&' \
           f'data_json={get_data(_step)}'
    async with aiohttp.ClientSession() as client:
        response = await client.post(url, data=data, headers=head)
        response = await response.json()
        now = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
        print(f"{_user[:3]}****{_user[-4:]}: [{now}] 修改步数（{_step}）" + response['message'])
